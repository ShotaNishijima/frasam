...
select_sigma_grid = function(
  ...
  tbl_sigma = tibble("stage" = 0, "Age" = NA,"AIC" = samres2$aic,
                     ...
                     "maxSE" = max(sqrt(diag(samres2$rep$cov.fixed))))
  for(i in 1:nrow(grid)) {
    ...
    if( isTRUE(check_converge)) {
      ...
      convergence_aic = tibble(
        ...
        maxSE = sapply(stage_sigma,function(X) max(sqrt(diag(X$rep$cov.fixed))))
      ) %>% mutate(ID = 1:n())
  ...
  if(isTRUE(check_converge)) {
    tbl_sigma = tbl_sigma %>% group_by(stage,convergence,pdHess) %>%
      mutate(model = ifelse(AIC == min(AIC)  & convergence==0 & pdHess == TRUE & maxSE <= SEmax,
                            "selected",NA))
  } else {
    tbl_sigma = tbl_sigma %>% group_by(stage) %>%
      mutate(model = ifelse(AIC == min(AIC),"selected",NA))
