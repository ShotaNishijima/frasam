...
- \code{boo_sam()}で実行できる
- ノンパラメトリックブートストラップ（\code{method="n"}）は厳密ではないので、パラメトリックブートストラップを推奨（\code{method="p"}）
- delta法で求めたCIも図に加える場合は\code{draw_deltaCI=TRUE}

```{r bootstrap}

res_boot <- boo_sam(res_sam2, n=100,method="p",seed=1)
...
```{r cross_test}
# pseudo dataを生成
pdata <- popsim_vpasam(res_sam2, n=100)
res_vpa_list <- list()
